...
class Me_MailForm
{
    ...
    private function entry()
    {
        ...
        // ステップパラメータをフォームに追加
        $form->innertext .= $this->get_hidden_tag(self::STEP_PARAMETER, $this->get_next_step());

        // tokenをセット
        $form->innertext .= $this->get_token();
    ...
    private function confirm()
    {
        ...
        // hidden要素をフォームに追加
        $form->innertext .= $this->get_hidden_tags();
    ...
    private function write_csv($file_path, $csv_lines)
    {
		...
		if (flock($handle, LOCK_EX)) {
            foreach ($csv_lines as $csv) {
                mb_convert_variables('sjis-win', self::SYSTEM_CHAR_CODE, $csv);
                fputcsv($handle, $csv);
            }
		}
